 ...
 *
 * @see com.github.panpf.zoomimage.compose.common.test.zoom.ZoomableStateImplTest.testRememberZoomableState
 */
@Composable
fun rememberZoomableState(logger: Logger = rememberZoomImageLogger()): ZoomableState {
    ...
    val zoomableState = remember(logger, layoutDirection) {
        ZoomableStateImpl(logger, layoutDirection)
    }
 ...
 *
 * @see com.github.panpf.zoomimage.compose.common.test.zoom.ZoomableStateImplTest
 */
@Stable
internal class ZoomableStateImpl(
    ...
    override var lastScaleAnimatable: Animatable<*, *>? = null
    override var lastFlingAnimatable: Animatable<*, *>? = null
    override var lastInitialUserTransform: Transform = Transform.Origin
    override var rotation: Int by mutableIntStateOf(0)
    override var rememberedCount = 0
     ...
     */
    override var alignment: Alignment by mutableStateOf(Alignment.Center)

    /**
     * Setup whether to enable read mode and configure read mode
     */
    override var readMode: ReadMode? by mutableStateOf(null)

    /**
     * Set up [ScalesCalculator] for custom calculations mediumScale and maxScale
     */
    override var scalesCalculator: ScalesCalculator by mutableStateOf(ScalesCalculator.Dynamic)
     ...
     */
    override var threeStepScale: Boolean by mutableStateOf(false)
     ...
     */
    override var rubberBandScale: Boolean by mutableStateOf(true)

    /**
     * One finger double-click and hold the screen and slide up and down to scale the configuration
     */
    override var oneFingerScaleSpec: OneFingerScaleSpec by mutableStateOf(OneFingerScaleSpec.Default)

    /**
     * The animation configuration for the zoom animation
     */
    override var animationSpec: ZoomAnimationSpec by mutableStateOf(ZoomAnimationSpec.Default)

    /**
     * Whether to limit the offset of the user's pan to within the base visible rect
     */
    override var limitOffsetWithinBaseVisibleRect: Boolean by mutableStateOf(false)

    /**
     * Add whitespace around containers based on container size
     */
    override var containerWhitespaceMultiple: Float by mutableStateOf(0f)

    /**
     * Add whitespace around containers, has higher priority than [containerWhitespaceMultiple]
     */
    override var containerWhitespace: ContainerWhitespace by mutableStateOf(ContainerWhitespace.Zero)
     ...
     */
    override var disabledGestureTypes: Int by mutableIntStateOf(0)

    /**
     * Whether to reverse the scale of the mouse wheel, the default is false
     */
    override var reverseMouseWheelScale: Boolean by mutableStateOf(false)

    /**
     * Zoom increment converter when zooming with mouse wheel
     */
    override var mouseWheelScaleScrollDeltaConverter: (Float) -> Float = { it * 0.33f }
     ...
     */
    override var baseTransform: Transform by mutableStateOf(Transform.Origin)
        private set
     ...
     */
    override var userTransform: Transform by mutableStateOf(Transform.Origin)
        private set
     ...
     */
    override var continuousTransformType: Int by mutableIntStateOf(0)

    override var lastContainerSize: IntSize = containerSize
    override var lastContentSize: IntSize = contentSize
    override var lastContentOriginSize: IntSize = contentOriginSize
    override var lastContentScale: ContentScale = contentScale
    override var lastAlignment: Alignment = alignment
    override var lastRotation: Int = rotation
    override var lastReadMode: ReadMode? = readMode
    override var lastScalesCalculator: ScalesCalculator = scalesCalculator
    override var lastLimitOffsetWithinBaseVisibleRect: Boolean = limitOffsetWithinBaseVisibleRect
    override var lastContainerWhitespace: ContainerWhitespace = calculateContainerWhitespace()
    ...
    override suspend fun reset(caller: String): Unit = coroutineScope {
        ...
        this@ZoomableStateImpl.lastInitialUserTransform = newInitialZoom.userTransform.toPlatform()
        this@ZoomableStateImpl.lastContainerSize = containerSize
        this@ZoomableStateImpl.lastContentSize = contentSize
        this@ZoomableStateImpl.lastContentOriginSize = contentOriginSize
        this@ZoomableStateImpl.lastContentScale = contentScale
        this@ZoomableStateImpl.lastAlignment = alignment
        this@ZoomableStateImpl.lastReadMode = readMode
        this@ZoomableStateImpl.lastRotation = rotation
        this@ZoomableStateImpl.lastScalesCalculator = scalesCalculator
        this@ZoomableStateImpl.lastLimitOffsetWithinBaseVisibleRect = limitOffsetWithinBaseVisibleRect
        this@ZoomableStateImpl.lastContainerWhitespace = containerWhitespace
    }
     ...
     */
    override suspend fun rotate(targetRotation: Int): Unit = coroutineScope {
     ...
     */
    override fun getNextStepScale(): Float {
     ...
     */
    override fun touchPointToContentPoint(touchPoint: Offset): IntOffset {
     ...
     */
    override fun canScroll(horizontal: Boolean, direction: Int): Boolean =
    ...
    override suspend fun rollbackScale(centroid: Offset?): Boolean = coroutineScope {
        val containerSize = containerSize.takeIf { it.isNotEmpty() } ?: return@coroutineScope false
        contentSize.takeIf { it.isNotEmpty() } ?: return@coroutineScope false
        val minScale = minScale
        val maxScale = maxScale
        ...
        if (targetScale != null) {
            ...
            this@ZoomableStateImpl.lastScaleAnimatable = updateAnimatable
            ...
            try {
                updateAnimatable.animateTo(
                    ...
                    val nowScale = this@ZoomableStateImpl.transform.scaleX
    ...
    override suspend fun fling(velocity: Velocity, density: Density): Boolean = coroutineScope {
        containerSize.takeIf { it.isNotEmpty() } ?: return@coroutineScope false
        contentSize.takeIf { it.isNotEmpty() } ?: return@coroutineScope false
        val currentUserTransform = userTransform
        ...
        this@ZoomableStateImpl.lastFlingAnimatable = flingAnimatable
        var job: Job? = null
        job = coroutineScope {
            launch {
                ...
                try {
                    ...
                    flingAnimatable.animateDecay(
                        ...
                        val currentUserTransform2 = this@ZoomableStateImpl.userTransform
    ...
    override fun checkSupportGestureType(@GestureType gestureType: Int): Boolean =
        disabledGestureTypes.and(gestureType) == 0

    override fun limitUserScale(targetUserScale: Float): Float {
    ...
    override fun limitUserScaleWithRubberBand(targetUserScale: Float): Float {
        val minUserScale = minScale / baseTransform.scaleX
        val maxUserScale = maxScale / baseTransform.scaleX
    ...
    override fun limitUserOffset(userOffset: Offset, userScale: Float): Offset {
    ...
    override suspend fun animatedUpdateUserTransform(
        ...
        val finalAnimationSpec = animationSpec ?: this@ZoomableStateImpl.animationSpec
        try {
            updateAnimatable.animateTo(
                targetValue = 1f,
                animationSpec = tween(
                ...
                this@ZoomableStateImpl.userTransform = userTransform
                updateTransform()
            }
        } catch (e: CancellationException) {
            throw e
    ...
    override fun calculateContainerWhitespace(): ContainerWhitespace {
